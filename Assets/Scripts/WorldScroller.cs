...
public class WorldScroller : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(ground2.transform.position.x < 0)
        {
            ...
            WorldSegment segmentPrefab = prefabSegments[Random.Range(0, prefabSegments.Length)];
            float d = ground2.sizeX * 0.5f + segmentPrefab.sizeX * 0.5f;
            Vector3 newPos = ground2.transform.position + new Vector3(d, 0, 0);
            WorldSegment newSegment = Instantiate(segmentPrefab, newPos, Quaternion.identity);
